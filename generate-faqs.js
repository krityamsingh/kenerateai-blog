...
const css=`:root{--bg:#090b10;--panel:#11151f;--text:#f5f7fb;--muted:#a9b0bf;--accent:#86a9ff;--line:#242b39;--max:1040px}*{box-sizing:border-box}body{margin:0;font:16px/1.6 system-ui,-apple-system,Segoe UI,sans-serif;background:#090b10;color:var(--text)}a{color:#b9cbff}.wrap{max-width:var(--max);margin:auto;padding:0 22px}header{border-bottom:1px solid var(--line);padding:18px 0;position:sticky;top:0;background:#090b10eb;z-index:5}.brand{font-weight:800;text-decoration:none;color:#fff}.hero{padding:60px 0 30px}h1{font-size:clamp(38px,7vw,70px);line-height:1.02;letter-spacing:-.04em;margin:10px 0 18px}.lead{color:var(--muted);max-width:760px}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px}.card,details{border:1px solid var(--line);background:var(--panel);border-radius:14px}.card{padding:20px;text-decoration:none}.card p,details p{color:var(--muted)}details{padding:16px 18px;margin:12px 0}summary{font-weight:750;cursor:pointer}.pager{display:flex;gap:12px;flex-wrap:wrap;margin:30px 0}.pager a{padding:10px 14px;border:1px solid var(--line);border-radius:10px;text-decoration:none}footer{border-top:1px solid var(--line);margin-top:50px;padding:30px 0;color:var(--muted)}@media(max-width:700px){.grid{grid-template-columns:1fr}}`;
function pageHead(title,desc,canonical){return `<!doctype html><html lang="en-US"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(desc)}"><link rel="canonical" href="${canonical}"><meta name="robots" content="index,follow,max-snippet:-1"><style>${css}</style></head><body><header><div class="wrap"><a class="brand" href="/">KenerateAI Creator Lab</a> · <a href="/faqs/">1,000 FAQs</a></div></header>`}
const faqDir=path.join(OUT,'faqs'); fs.mkdirSync(faqDir,{recursive:true});
let hub=pageHead('1,000 KenerateAI FAQs — Creator, SEO, GEO & AI Guides','A searchable library of 1,000 practical KenerateAI questions and concise answers across image, video, prompting, editing, pricing, SEO, GEO, privacy and troubleshooting.',`${BASE}/faqs/`);
hub+=`<main><section class="hero"><div class="wrap"><div>FAQ Library · 1,000 answers</div><h1>1,000 KenerateAI FAQs</h1><p class="lead">Ten focused sections with 100 questions each. Answers are written for people first and rendered as crawlable static HTML.</p><div class="grid">`;
for(const [ti,t] of topics.entries()){
  ...
  let html=pageHead(`${t.title}: 100 KenerateAI FAQs`,`100 practical questions and concise answers about ${t.title.toLowerCase()} in KenerateAI.`,`${BASE}/faqs/${t.slug}`);
  html+=`<main><section class="hero"><div class="wrap"><div>FAQ section ${ti+1} of 10</div><h1>${esc(t.title)}</h1><p class="lead">100 questions and answers. FAQ numbers ${ti*100+1}–${ti*100+100} of the 1,000-question library.</p></div></section><section><div class="wrap">`;
  faqs.forEach((f,i)=>{html+=`<details${i<3?' open':''}><summary>${ti*100+i+1}. ${esc(f.q)}</summary><p>${esc(f.a)}</p></details>`});
  html+=`<div class="pager"><a href="/faqs/">← FAQ hub</a>${ti>0?`<a href="/faqs/${topics[ti-1].slug}">Previous section</a>`:''}${ti<topics.length-1?`<a href="/faqs/${topics[ti+1].slug}">Next section</a>`:''}</div></div></section></main><footer><div class="wrap">KenerateAI Creator Lab FAQ Library · ${faqs.length} questions in this section</div></footer></body></html>`;
  fs.writeFileSync(path.join(faqDir,file),html);
  hub+=`<a class="card" href="/faqs/${t.slug}"><strong>${esc(t.title)}</strong><p>100 FAQs · #${ti*100+1}–${ti*100+100}</p></a>`;
}
hub+=`</div></div></section></main><footer><div class="wrap">Exactly 1,000 FAQs across 10 sections.</div></footer></body></html>`; fs.writeFileSync(path.join(faqDir,'index.html'),hub);
...
let llms=fs.existsSync(path.join(OUT,'llms.txt'))?fs.readFileSync(path.join(OUT,'llms.txt'),'utf8'):'';
llms+=`\n\n## 1,000 FAQ library\n- [FAQ hub](${BASE}/faqs/): 1,000 practical KenerateAI FAQs.\n${topics.map(t=>`- [${t.title}](${BASE}/faqs/${t.slug}): 100 FAQs.`).join('\n')}\n`;
fs.writeFileSync(path.join(OUT,'llms.txt'),llms);
console.log('Generated FAQ count:',topics.length*100,'pages:',topics.length+1,'output:',OUT);
